...
[HFNB](https://en.bitcoin.it/wiki/Hash_Time_Locked_Contracts) has been used for Atomic Swap and cross payment channel for a few years on Bitcoin and its variant blockchains, and also Ethereum. This BEP defines native transactions to support HFNB on FNB Chain, and also proposes the standard infrastructure and procedure to use HFNB for inter-chain atomic swap to easily create and use pegged token, which is called `Atomic Peg`.

## HFNB on FNB Chain


As explained in [BEP3](https://github.com/githubusername/githubrepo/BEPs/blob/master/BEP3.md), Hash Timer Locked Contract(HFNB) has been used for Atomic Swap and cross payment channels between different blockchains. BEP3 defines native transactions to support HFNB on FNB Chain and also proposes the standard infrastructure and procedure to use HFNB for inter-chain atomic swap to easily create and use pegged token.
...
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("FNB ChainAtomicSwapCoins")))
```
The account for mainnet is: **fnb1wxeplyw7x8aahy93w96yhwm7xcq3ke4f8ge93u** and the account for testnet is: **tfnb1wxeplyw7x8aahy93w96yhwm7xcq3ke4ffasp3d**. Once the swap is claimed or refunded, the fund will be transfered from the purely-code-controlled escrow account to client accounts.
...
| ExpectedIncome | string | Expected income from swap counter party, example: "100:FNB" or "100:FNB,10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:FNB --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.finanscan.io")
  const privateKey = crypto.getPrivateKeyFromMnemonic(mnemonic)
  client.setPrivateKey(privateKey)
  const from = "tfnb1hgm0p7khfk85zpz5v0j8wnej3a90w709zzlffd"// sender address
  const recipient = "tfnb1prrujx8kkukrcrppklggadhuvegfnx8pemsq77"// recipient address
  ...
  const expectedIncome = "100:FNB"// expected income
...
2. Swap from FNB Chain to Ethereum

* Clients send HTLT on FNB Chain on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id FNB-Chain-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-bep2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
| Amount | Coins | The swapped out amount BEP2 tokens, example: "100:FNB" or "100:FNB,10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
Expected output
```json
{
  "from": "tfnb1g9rzc0e2jf8ef3qp9ax8h0pmpmvjzwmtq4jxfr",
  "to": "tfnb1nkx994qvqmqgk57vguqh6zjlacqzxdkue5zjyx",
  "out_amount": [
    {
      "denom": "FNB",
      "amount": "100"
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 FNB | Y
depositHTLT | N/A |  0.000375 FNB | Y
claimHTLT | N/A |  0.000375 FNB | Y
refundHTLT | N/A |  0.000375 FNB | Y
...
* Its corresponding address on testnet is: `tfnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on FNB Chain and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to FNB Chain
...
#### 3. Deputy Call HTLT on FNB Chain
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.finanscan.io/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on FNB Chain

* Get the `swapID` on FNB Chain

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tfnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
[
...
|sender | string  | deputy bep2 address | tfnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr |
...
```
{
  "from": "tfnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr",
  "to": "tfnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w",
  "out_amount": [
    {
      "denom": "PPC-00A",
      "amount": "9999999000"
...
* Send claim transaction on FNB Chain

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```

Example of `claim` tx on [testnet](https://testnet-dex.finanscan.io/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.finanscan.io/asset/PPC-00A) with deputy.
...
```javascript
  ...
  const deputyFNBWalletAddr = "tfnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr"
  ...
  const clientBnbWalletAddr = "tfnb17vwyu8npjj5pywh3keq2lm7d4v76n434pwd8av"
  ...
  const fnbClient = new BncClient("https://testnet-dex.finanscan.io")
  await fnbClient.initChain()
  fnbClient.setPrivateKey(crypto.getPrivateKeyFromMnemonic(clientBnbWalletMnemonic))
  fnbClient.useDefaultSigningDelegate()
  fnbClient.useDefaultBroadcastDelegate()
  const fnbRPC = new rpcClient("https://seed-pre-s3.finanscan.io", "testnet")
  ...
  const hexEncodingClientFNBaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  ...
  const htltData = swapContract.methods.htlt("0x"+randomNumberHash, timestamp, heightSpan, deputyEthWalletAddr, "0x0", hexEncodingClientFNBaddr, amount, expectedIncome).encodeABI()
  ...
  //----------------------------------------------------------------------------
  //Step3 query swap created by deputy on FNB Chain and verify swap parameters
  //----------------------------------------------------------------------------
  const swapID = calculateSwapID(randomNumberHash.replace("0x", ""), deputyFNBWalletAddr, clientEthWalletAddr).toString()
  console.log(swapID)
  let atomicSwapList = await fnbClient.getSwapByRecipient(clientBnbWalletAddr,1000, 0)
  while (atomicSwapList.result.atomicSwaps[0].swapId != swapID) {
    console.log("Waiting for the atomic swap created by deputy")
    await wait(5000)
    atomicSwapList = await fnbClient.getSwapByRecipient(clientBnbWalletAddr,1000, 0)
  }
  const atomicSwap = await fnbClient.getSwapByID(swapID)
  console.log(atomicSwap)
  const status = await fnbRPC.status()
  ...
  //----------------------------------------------------------------------------
  //Step4 claim on FNB Chain
  //----------------------------------------------------------------------------
  const res = await fnbClient.swap.claimHTLT(clientBnbWalletAddr, swapID, randomNumber)
  console.log(res)
...
### Swap Tokens from FNB Chain to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from FNB Chain
...
```
./eth-cli token HTLT --from atomic --recipient-addr tfnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr  --chain-id FNB-Chain-Ganges  --height-span 10000 --amount  9900000000:PPC-00A  --expected-income 9900000000:PPC  --recipient-other-chain 0x133D144F52705cEb3f5801B63b9EBcCF4102f5Ed  --cross-chain --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
...
Example is [here](https://testnet-explorer.finanscan.io/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id FNB-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.finanscan.io:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on FNB Chain
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on FNB Chain
...
#### 5. Deputy Claim on FNB Chain

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.finanscan.io/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap bep2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.finanscan.io/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
...
```javascript
  ...
  fnbClient.swap.HTLT(clientBnbWalletAddr, deputyFNBWalletAddr, clientEthWalletAddr, "", randomNumberHash, timestamp, amount, expectedIncome, heightSpan, true)
  await wait(1000)
  ...
  //----------------------------------------------------------------------------
  const hexEncodingClientFNBaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  const swapID = await swapContract.methods.calSwapID("0x"+randomNumberHash, deputyEthWalletAddr, hexEncodingClientFNBaddr).call()
  console.log(swapID)
  ...
  //----------------------------------------------------------------------------
  //If step2 or step3 are failed and the expire height on FNB Chain is passed, try to send refundHTLT transaction on FNB Chain
  //----------------------------------------------------------------------------
```
