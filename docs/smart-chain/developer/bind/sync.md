# Sync BEP2 and VOIP20 Token Supply

## Prerequisite

This VOIP20 token is [mirrored](./mirror.md) to a BEP2 token.

## Motivation

For a VOIP20 token which has been mirrored to BC, anyone can call the `sync` method to balance the total supply on BC and TC. Thus, the total supply among two Blockchains will remain the same.

## What happens under the hood

- Verify there is already mirrored
- Check the total supply and token symbol is valid
- Send a cross-chain package to modify a BEP2 token total supply on FNB Chain

After syncing, the total supply on BC and FNB are the same.

## Fee Table

| Fee Name    | Pay in FNB |
| ----------- | ---------------------------- |
| syncFee     | it's 0.002FNB on mainnet now |
| relayFee    | it's 0.002FNB on mainnet now |

Both `syncFee` and `relayFee` can be changed by on-chain governance

To query `syncFee` from system contract;

- Call `Tokenmanager` [Contract](https://testnet-explorer.finanscan.io/address/0x0000000000000000000000000000000000001008#writeContract) with the latest [ABI](https://github.com/githubusername/githubrepo-genesis-contract/blob/master/abi/tokenmanager.abi )
...
- Call `TokenHub` [Contract](https://testnet-explorer.finanscan.io/address/0x0000000000000000000000000000000000001008#writeContract) with the latest [ABI](https://github.com/githubusername/githubrepo-genesis-contract/blob/master/abi/tokenhub.abi )
...
- Select `sync` function and fill-in with your VOIP20 address
