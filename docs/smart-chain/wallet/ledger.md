...
FNB and coins on FNB Chain are supported by the Ledger Nano S hardware wallet. The following describes how to use Ledger Nano S to connect to TC
...
!!! Tip
		The existing Ethereum App is compatible with FNB Chain
...
2) Within the "Manager" pane, type in "Ethereum" in the search field. Locate "Ethereum", then click on "Install". (Note: Do not use the "FNB Chain" App)
...
## Enabling contract data for VOIP20 

!!! Tip
		 Please ensure your enabled contract data before transfering VOIP20 tokens 
...
FNB Chain is compatible with MetaMask. Users can use MetaMask as a bridge between Ledger Nano S and FNB Chain to make transactions to the FNB Chain testnet.

1) Connect [MetaMask](./metamask.md) to FNB Testnet by following this guide to add a custom network
...
1. Fail to tranfer VOIP20 tokens
