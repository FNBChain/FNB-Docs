...
This document explains how the genesis file of the FNB Chain mainnet is structured. It also explains how you can build a genesis file for your own testnet.

Note that you can generate a default genesis file for your own testnet by running the following command:

```
fnbchaind init  --chain-id
```

The genesis file is stored in `~/.fnbchaind/config/genesis.json`.
...
```
"chain_id": "FNB-Chain-Tigris",
```
## Consensus Parameters

Next, the genesis file defines consensus parameters. Consensus parameters regroup all the parameters that are related to the consensus layer, which is `Tendermint` in the case of `FNB Chain`. Let us look at these parameters:
...
[
  {
  name: "Aconcagua",
  address: "fnb1y888axmhzz6yjj464syfy68mkhzy9phlv8fzac",
  consensus_addr: ""
  },
  {
  name: "Aconcagua",
  address: "fnb1kdx4xkktr35j2mpxncvtsshswj5gq577me7lx4",
...
- The first address `fnb1y888axmhzz6yjj464syfy68mkhzy9phlv8fzac`. At genesis block, according to `gentx` info, same amount of staking tokens will be allocated to this address. It is also used to receive rewards after chain launched.
- The second address`fnb1kdx4xkktr35j2mpxncvtsshswj5gq577me7lx4`is used for governance
- `consensus_addr`is used for signing new blocks

### Token

The `tokens` module handles tokens. It contains the information of FNB Chain native token `FNB`
```
name: "FNB Chain Native Token",
symbol: "FNB",
total_supply: "20000000000000000",
owner: "fnb1ultyhpw2p2ktvr68swz56570lgj2rdsadq3ym2",
mintable: false
```
...
delegator_address: "fnb1y888axmhzz6yjj464syfy68mkhzy9phlv8fzac",
