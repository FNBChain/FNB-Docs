...
!!! warning
    You need to install [fnbchaind](./install.md) before you go further
...
* Your full node has to run at least 4 hours per 24 hours in order to catch up with FNB Chain More hours will be better, run your node continuously for best results.

## Setting Up a New Node
> You can skip this part if you have executed the install script.

First you need to choose a home folder `$BNCHOME` (i.e. ~/.fnbchaind) for FNB Chain.
You can setup this by:

```
mkdir ~/.fnbchaind
mkdir ~/.fnbchaind/config
```
...
### Add Seed Nodes
Your Full Node needs to know how to find peers in the blockchain network. You'll need to add healthy seed nodes to $HOME/.fnbchain/config/config.toml. The recommended `config.toml` already contains links to some seed nodes.

If those seeds aren't working, you can find more seeds and persistent peers in HTTP API endpoints: https://dex.finanscan.io/api/v1/peers
...
- Log: The log file is under `home`- the directory specified when starting `fnbchaind`.
...
* [Explorer](https://explorer.finanscan.io/)
