...
Currently, the cost to get a BEP2 token listed on FNB DEX is about 1500FNB, which is a big barrier to many small, promising projects. As explained in [BEP8](https://github.com/githubusername/githubrepo/BEPs/blob/master/BEP8.md),the idea of BEP8 Tokens is raised to accommodate the small or micro projects, intellectual properties, and other small token economies to build a more comprehensive venue for token trading.

Based on the limitation usage of the network, the cost of issuing and listing of such BEP8 tokens are minimized to a different magnitude. Besides, any BEP8 token issuer can choose to directly list against FNB and BUSD without the voting process of validators. Stablecoin pairs of BEP8 will provide a UX advantage and make any two BEP82 tokens exchangeable.
...
* Owner: The initial issuer of this token, the FNB balance of issuer should be more than the fee for issuing tokens
...
```bash
./eth-cli token issue-tiny --home $home -s $symbol --token-name $token_name -n 10000 --mintable --from $from --token-uri http://www.example.com --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443 --trust-node
```

```bash
./eth-cli token issue-mini --home $home -s $symbol --token-name $token_name -n 1000000 --mintable --from $from --token-uri http://www.example.com --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443 --trust-node
```
...
Example on **mainnet**:
```bash
./eth-cli send --from $from-key-alias --to to-address --amount 200000000:mini-token-symbol --chain-id FNB-Chain-Tigris --node  https://dataseed5.defibit.io:443 --json
```
...
```bash
./eth-cli token freeze --amount 200000000 --symbol $mini-token-symbol --from alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443 --trust-node
```
...
```bash
./eth-cli token unfreeze --amount 200000000 --symbol $mini-token-symbol --from alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```
...
```bash
./eth-cli token mint --amount 10000000000 --symbol $mini-token-symbol --from alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```
...
```bash
./eth-cli token burn --amount 100000000000 --symbol $mini-token-symbol --from alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```
...
```bash
./eth-cli token set-uri-mini --symbol $mini-token-symbol --token-uri http://www.efg.com --from alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```

### List BEP8 Tokens on FNB DEX

The listing process is different from BEP2 token list. BEP8 token issuer can list the Mini-BEP2 token without the agreement of FNB Chain validators. The BEP8 token can only be listed against FNB or BUSD and they cannot be listed as a quote symbol.
...
|quote-asset-symbol| string|only support FNB and BUSD as quote asset|
...
```bash
./eth-cli dex list-mini -s=$mini-token-symbol --quote-asset-symbol=FNB --init-price=1000000000 --from=alice --chain-id FNB-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```

## BEP8 Token Trading Pair Matching

The number of BEP8 trading pairs could be much bigger than BEP2 pairs due to the cheap issue/listing fee. As they are designed to use limited network resource, FNB DEX match engine will only allocate a fixed resource on matching the BEP8 pairs. Unlike BEP2 token, BEP8 trading pairs are not matched every block, but they are guaranteed to be matched at least once in every 16 blocks.
...
### Delist
Delist a trading pair of Mini-BEP2 token is the same as BEP2 [delist](). It needs the agreement of FNB Chain validators.

## Fee Table for Testnet

API URL: <https://testnet-dex.finanscan.io/api/v1/fees?format=amino>
