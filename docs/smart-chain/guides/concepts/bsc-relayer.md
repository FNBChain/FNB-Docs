...
Relayers for BC-to-NCcommunication referred to as **NCRelayers** are a standalone process that can be run by anyone, and anywhere, except that Relayers must register themselves onto FNB and deposit a certain amount of FNB. Only relaying requests from the registered Relayers will be accepted by TC.
...
As a FNB relayer, it must have proper configurations on the following three items:
...
A FNB relayer is required to parse all block results and pick out all events with event type “IBCPackage” from endBlock event table. This is an cross chain package event example:
