# Run a light client to join FNB Chain

Light client is a program that connects to a full node to help users access and interact with FNB Chain
...
Flags:
      ...
      --chain-id string            Specify the shree chain ID (default "fnbchain")
...
You can refer to [Run a FNB Chain full node](fullnode.md) to get more details.<br/>
...
d = requests.get('https://dex.finanscan.io/api/v1/peers').text # replace dex.finanscan.io with testnet-dex.finanscan.io for testnet
l = json.loads(d)
...
### Example for Mainnet:
```bash
./lightd --chain-id "FNB-Chain-Tigris" --node tcp://dataseed1.finanscan.io:80 > node.log  &
```

### Example for Testnet:
```bash
./lightd --chain-id "FNB-Chain-Ganges" --node tcp://data-seed-pre-0-s1.finanscan.io:80 > node.log  &
```
